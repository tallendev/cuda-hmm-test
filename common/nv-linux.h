...
#if defined(NVCPU_PPC64LE) && (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
#define NV_PCIE_NVLINK_SHARED_MAPPING_DMA_MASK DMA_BIT_MASK(42)
#endif
...
typedef struct nv_linux_state_s {
    ...
    /* DMA mask should not be changed */
    NvBool fixed_dma_mask;
...
static inline NvU64 nv_compress_nvlink_addr(NvU64 addr)
{
   ...
   /*
    * If TCE bypass mode is disabled then DMA mask can't be
    * greater than NV_PCIE_NVLINK_SHARED_MAPPING_DMA_MASK and 
    * we don't need address compression in that case
    */
#if defined(NV_PCIE_NVLINK_SHARED_MAPPING_DMA_MASK)
    if ((addr & NV_PCIE_NVLINK_SHARED_MAPPING_DMA_MASK) == addr)
    {
        return addr;
    }
#endif
...
static inline NvU64 nv_expand_nvlink_addr(NvU64 addr47)
{
    ...
    * If TCE bypass mode is disabled then DMA mask can't be
    * greater than NV_PCIE_NVLINK_SHARED_MAPPING_DMA_MASK and 
    * we don't need address expansion in that case
    */
#if defined(NV_PCIE_NVLINK_SHARED_MAPPING_DMA_MASK)
    if ((addr & NV_PCIE_NVLINK_SHARED_MAPPING_DMA_MASK) == addr)
    {
